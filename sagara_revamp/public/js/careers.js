        ...
        container.innerHTML = jobData.map(job => `
            <div class="job-card bg-white dark:bg-slate-800/50 rounded-2xl p-6 border border-slate-100 dark:border-slate-700 cursor-pointer" onclick="window.location.href='/job-detail.html?id=${job.id}'">
                <div class="flex items-start justify-between mb-4">
                    <div>
                        <h3 class="text-xl font-bold mb-1 hover:text-primary transition-colors">${job.title || 'Specialist Position'}</h3>
                ...
                <p class="text-slate-500 dark:text-slate-400 text-sm mb-4 line-clamp-2">${job.description ? job.description.substring(0, 120) : 'Join our team and help us build the future of technology...'}...</p>
