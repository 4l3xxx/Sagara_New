            ...
            <div class="blog-card bg-white dark:bg-slate-800/50 rounded-2xl overflow-hidden border border-slate-100 dark:border-slate-700 cursor-pointer" onclick="window.location.href='/blog-detail.html?id=${blog.id}'">
                ...
                <div class="p-6">
                    <div class="flex items-center gap-2 text-xs text-primary mb-3">
                        <span class="material-symbols-outlined text-sm">schedule</span> ${new Date(blog.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
                    </div>
                    <h3 class="text-xl font-bold mb-2 hover:text-primary transition-colors">${blog.title}</h3>
                    <p class="text-slate-500 dark:text-slate-400 text-sm line-clamp-3">${blog.excerpt || blog.content.substring(0, 150)}...</p>
